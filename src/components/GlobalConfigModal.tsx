...
import { Modal, Input, Button, message, Space, Typography, Alert, Spin } from 'antd';
import { ReloadOutlined, UndoOutlined, CheckCircleOutlined, ExclamationCircleOutlined } from '@ant-design/icons';
...
export const GlobalConfigModal: React.FC<GlobalConfigModalProps> = ({
  ...
  const { config, updateConfig, resetConfig, reloadConfig, isLoading } = useGlobalConfig();
  ...
  const validateJson = useCallback((text: string) => {
    try {
      ...
      setValidationResult({
        isValid: false,
        errors: [`JSON格式错误: ${(error as Error)?.message || '未知错误'}`]
      });
      return false;
    }
  }, []);
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      ...
      message.error(`应用配置失败: ${(error as Error)?.message || '未知错误'}`);
    }
