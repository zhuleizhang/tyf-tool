...
export const useImageOCR = (
  images: ImageData[],
  updateImageText: (id: string, text: string, status?: ImageData['status'], confidence?: number) => void
) => {
  ...
  const pendingCount = useMemo(() => 
    images.filter(img => img.status === 'pending' || img.status === 'error').length,
    [images]
  );
  ...
  // 获取缓存键
  const getCacheKey = useCallback((imageUrl: string, fileSize: number): string => {
  ...
  const recognizeImage = useCallback(async (imageId: string, options: any = {}): Promise<OCRResult> => {
    ...
    try {
      ...
      if (cached && Date.now() - cached.timestamp < CACHE_EXPIRY) {
        ...
        updateImageText(imageId, cached.result.text, 'completed', cached.result.confidence);
      ...
      // 更新文本和状态
      updateImageText(imageId, result.text, 'completed', result.confidence);
      ...
      setOCRStats(prev => ({
        ...
        avgConfidence: (prev.avgConfidence * prev.totalProcessed + result.confidence) / (prev.totalProcessed + 1)
      }));
      
      // 显示识别结果信息
      console.log(`OCR完成 - 置信度: ${(result.confidence * 100).toFixed(1)}%, 词数: ${result.words}, 耗时: ${processingTime}ms`);
      ...
      if (errorMessage.includes('网络')) {
        ...
        message.error('图片格式不支持，请使用JPG、PNG等常见格式', 5);
      } else {
  ...
  const recognizeAll = useCallback(async (options: any = {}) => {
    if (images.length === 0) {
    ...
    try {
      ...
      if (window.electronAPI?.onBatchOCRProgress) {
        batchCleanup = window.electronAPI.onBatchOCRProgress((data: BatchOCRProgress) => {
          const currentProgress = (data.completed / data.total) * 100;
      ...
      for (let i = 0; i < pendingImages.length; i += concurrencyLimit) {
        ...
        const batchPromises = batch.map(async (image, index) => {
          try {
            ...
            const result = await recognizeImage(image.id, options);
            completed++;
            
            // 更新进度
            const currentProgress = (completed / pendingImages.length) * 100;
            setProgress(currentProgress);
            
            return { success: true, result, imageId: image.id };
          } catch (error) {
            console.error(`Failed to recognize image ${image.id}:`, error);
            completed++;
      ...
      if (successful > 0) {
        message.success(
          `批量识别完成: ${successful}张成功${failed > 0 ? `, ${failed}张失败` : ''}，耗时 ${(totalTime / 1000).toFixed(1)}秒`,
          8
        );
      } else {
      ...
      console.log(`批量OCR统计: 成功${successful}张，失败${failed}张，平均每张${avgTimePerImage.toFixed(0)}ms`);
